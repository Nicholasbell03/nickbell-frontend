...
const SPINNER_SVG = `<div class="flex items-center justify-center py-8 text-emerald-500 animate-pulse">
  ...
  renderQueue = renderQueue.then(async () => {
    for (const { node, id, text } of toRender) {
      try {
        ...
        node.innerHTML = `<div class="p-4 border border-red-500/20 bg-red-500/10 text-red-400 rounded-md text-sm">Failed to render diagram.</div>`;
      }
    }
  });
