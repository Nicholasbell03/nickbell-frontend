...
export const ChatMessage = ({ role, content }: ChatMessageProps) => {
  ...
  return (
    <div className={`flex gap-4 mb-6 animate-fade-in ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      <Avatar className={`h-10 w-10 flex-shrink-0 ${isUser ? 'ring-2 ring-emerald-500/30' : 'ring-2 ring-teal-500/30'}`}>
        <AvatarFallback className={`${isUser ? 'bg-gradient-to-br from-emerald-500 to-teal-500' : 'bg-gradient-to-br from-slate-700 to-slate-600'} text-white text-sm font-semibold`}>
